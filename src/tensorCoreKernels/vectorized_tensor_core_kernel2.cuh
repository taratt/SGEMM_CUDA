...
__global__ void runSgemmVectorizedTensorCore2(int M, int N, int K, float alpha, __half *A,
    ...
    const uint rowSharedLoaderB = (threadIdx.x ) / (BN / 8);
    const uint colSharedLoaderB = (threadIdx.x ) % (BN / 8);
